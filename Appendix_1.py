...
def run_solver():
    ...
    xdmf = XDMFFile('Numerical_Pressure_Gradient_V1.xdmf')
    xdmf.write(p)
    xdmf.close()

    xdmf = XDMFFile('Velocity_Profile_V1.xdmf')
    xdmf.write(u_bar)
    xdmf.close()

def test_solver():
    "Test solver by reproducing Pressure p = 1 - x^2 and generating plots"
    ...
    LogDOFa = np.log(DOFa)
    LogGPSa = np.log(GPSa)
    # the first number printed below is the gradient of the Log plot of L2 Error against cell size, h. This is the error convergence rate.
